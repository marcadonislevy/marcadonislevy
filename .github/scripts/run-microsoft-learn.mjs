...
const newLog = `${oldLog}\n    if (Array.isArray(item.data?.achievements)) {\n      console.log(\`Achievement API totalCount: \${item.data.totalCount ?? "unknown"}\`);\n      const sample = item.data.achievements.slice(0, 5).map((achievement) => ({\n        keys: Object.keys(achievement ?? {}),\n        type: achievement?.type ?? achievement?.achievementType ?? achievement?.kind ?? achievement?.category ?? null,\n        title: achievement?.title ?? achievement?.name ?? achievement?.displayName ?? null,\n      }));\n      console.log(\`Achievement API sample: \${JSON.stringify(sample)}\`);\n    }`;
...
const patched = source.replace(oldBlock, newBlock).replace(oldLog, newLog);
